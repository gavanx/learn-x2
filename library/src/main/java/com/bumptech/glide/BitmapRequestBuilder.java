...
/**
 * A class for creating a request to load a bitmap for an image or from a video. Sets a variety of type independent options including resizing,
 * animations, and placeholders.
 *
 * <p> Warning - It is <em>not</em> safe to use this builder after calling <code>into()</code>, it may be pooled and reused. </p>
 *
...
public class BitmapRequestBuilder<ModelType, TranscodeType> extends GenericRequestBuilder<ModelType, ImageVideoWrapper, Bitmap, TranscodeType>
    ...
    BitmapRequestBuilder(LoadProvider<ModelType, ImageVideoWrapper, Bitmap, TranscodeType> loadProvider, Class<TranscodeType> transcodeClass,
    ...
    /**
     * Load images at a size that is at most exactly as big as the target using {@link com.bumptech.glide.load.resource.bitmap.Downsampler#AT_MOST}.
     *
     * @return This request builder.
    ...
    /**
     * Load images using the given {@link Downsampler}. Replaces any existing image decoder. Defaults to {@link Downsampler#AT_LEAST}. Will be ignored
     * if the data represented by the model is a video. This replaces any previous calls to {@link #imageDecoder(ResourceDecoder)}  and {@link
     * #decoder(ResourceDecoder)} with default decoders with the appropriate options set.
     *
    ...
    /**
     * Loads and displays the {@link android.graphics.Bitmap} retrieved by the given thumbnail request if it finishes before this request. Best used
     * for loading thumbnail {@link Bitmap}s that are smaller and will be loaded more quickly than the fullsize {@link Bitmap}. There are no
     * guarantees about the order in which the requests will actually finish. However, if the thumb request completes after the full request, the
     * thumb {@link android.graphics.Bitmap} will never replace the full image.
     *
     * @param thumbnailRequest The request to use to load the thumbnail.
     * @return This request builder.
     * @see #thumbnail(float)
     *
     * <p> Note - Any options on the main request will not be passed on to the thumbnail request. For example, if you want an animation to occur when
     * either the full {@link android.graphics.Bitmap} loads or the thumbnail loads, you need to call {@link #animate(int)} on both the thumb and the
     * full request. For a simpler thumbnail option where these options are applied to the humbnail as well, see {@link #thumbnail(float)}. </p>
     *
     * <p> Only the thumbnail call on the main request will be obeyed, recursive calls to this method are ignored. </p>
     */
    ...
    /**
     * Sets the {@link com.bumptech.glide.load.ResourceDecoder} that will be used to decode {@link Bitmap}s obtained from an {@link
     * java.io.InputStream}.
     *
    ...
    /**
     * Sets the preferred format for {@link Bitmap}s decoded in this request. Defaults to {@link DecodeFormat#PREFER_RGB_565}. This replaces any
     * previous calls to {@link #imageDecoder(ResourceDecoder)}, {@link #videoDecoder(ResourceDecoder)}, {@link #decoder(ResourceDecoder)} and {@link
     * #cacheDecoder(com.bumptech.glide.load.ResourceDecoder)}} with default decoders with the appropriate options set.
     *
     * <p> Note - If using a {@link Transformation} that expect bitmaps to support transparency, this should always be set to ALWAYS_ARGB_8888.
     * RGB_565 requires fewer bytes per pixel and is generally preferable, but it does not support transparency. </p>
     *
     ...
     * {@inheritDoc}
     *
     * <p> Note - If no transformation is set for this load, a default transformation will be applied based on the value returned from {@link
     * android.widget.ImageView#getScaleType()}. To avoid this default transformation, use {@link #dontTransform()}. </p>
     *
